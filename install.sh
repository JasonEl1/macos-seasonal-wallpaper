...
if [[ $"folder_count" -gt 7 ]]; then
    ...
    if [[ $continue == "y" ]]; then
        if[ ! -f build/wallpaper-${sys_type}-${sys_arch} ]; then
        ...
        chmod +x build/wallpaper-${sys_type}-${sys_arch}
        ...
        cp seasons.json $wallpaper_path/../seasons.json
        echo "Copied seasons.json to wallpaper directory"
    else
